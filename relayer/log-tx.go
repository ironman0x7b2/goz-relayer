...
func (c *Chain) LogFailedTx(res sdk.TxResponse, err error, msgs []sdk.Msg) {
	...
	if res.Codespace != "" && res.Code != 0 {
		c.logger.Info(fmt.Sprintf("✘ [%s]@{%d} - err(%s:%d:%s)", c.ChainID, res.Height, res.Codespace, res.Code, res.RawLog))
	}
...
func (c *Chain) LogSuccessTx(res sdk.TxResponse, msgs []sdk.Msg) {
	c.logger.Info(fmt.Sprintf("✔ [%s]@{%d} - hash(%s)", c.ChainID, res.Height, res.TxHash))
}
